package app.web.orders.user.infrastructure.persistence;
...
class UserRepositoryImpl implements UserRepository {
    ...
    public boolean usernameIsUnique(String username) {
        ...
        Long amountOfRow = entityManager.createQuery(query, Long.class)
    ...
    public Optional<User> findByConfirmationToken(String confirmationToken) {
        ...
        Optional<UserEntity> result = entityManager.createQuery(query, UserEntity.class)
    ...
    public User findByIdOrThrowException(long userId) {
        return Optional.ofNullable(entityManager.find(UserEntity.class, userId))
                .map(UserRepository::createUser)
                .orElseThrow(() -> getElementNotFoundException(userId));
    }
    ...
    public User remove(User user) {
        ...
        UserEntity userEntity = Optional.ofNullable(entityManager.find(UserEntity.class, userId))
                .orElseThrow(() -> getElementNotFoundException(userId));
    ...
    public Optional<User> findByEmail(String email) {
        ...
        Optional<User> user = entityManager.createQuery(query, UserEntity.class)
                .setParameter("email", email)
                .getResultList()
                .stream()
                .findAny()
    ...
    public Optional<User> findByRestorePasswordToken(String restoreToken) {
        ...
        return entityManager.createQuery(query, UserEntity.class)
                .setParameter("restoreToken", restoreToken)
                .getResultList()
                .stream()
                .findAny()
                .map(UserRepository::createUser);
    ...
    public List<User> findAllByUserInfoRole(UserRole userRole) {
        return entityManager.createQuery("select user from UserEntity user where user.userRole=:userRole", UserEntity.class)
