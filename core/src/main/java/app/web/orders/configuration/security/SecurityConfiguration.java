package app.web.orders.configuration.security;
...
class SecurityConfiguration {
    ...
    public SecurityFilterChain createSecurityConfiguration(HttpSecurity httpSecurity) throws Exception {
        ...
        httpSecurity.authorizeHttpRequests(authorization -> {
            authorization
                    .requestMatchers("/api/users/register").permitAll().requestMatchers("/api/users/confirm").permitAll().requestMatchers("/api/users/password-restore").permitAll().requestMatchers("/api/users/set-new-password").permitAll().requestMatchers("/api/login").permitAll().requestMatchers("/api/home/open").permitAll().anyRequest().authenticated();
        });
